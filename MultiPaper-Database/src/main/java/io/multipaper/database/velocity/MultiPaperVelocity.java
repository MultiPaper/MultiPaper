package io.multipaper.database.velocity;
...
public class MultiPaperVelocity {
    ...
    public void onProxyInitialization(ProxyInitializeEvent event) {
        ...
        this.port = Math.toIntExact(config.getLong("port", Long.valueOf(MultiPaperDatabase.DEFAULT_PORT)));
        this.balanceNodes = config.getBoolean("balance-nodes", true);

        new MultiPaperDatabase(this.port);
    }
    ...
    public void onServerConnect(ServerPreConnectEvent event) {
        ...
        if (this.balanceNodes && isMultiPaperServer(targetServer.getServerInfo().getName())) {
            ...
            for (RegisteredServer server : servers) {
                ...
                ServerInfo multipaperServer = ServerInfo.getByName(serverName);

                if (multipaperServer != null && multipaperServer.isAlive()
                        && multipaperServer.getTimer().averageInMillis() < lowestTickTime) {
