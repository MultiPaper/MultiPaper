package io.multipaper.database.proxy;
...
public class ProxyServer extends Thread {
    ...
    private SocketAddress getSuitableServer() {
        ...
        for (ServerInfo server : ServerInfo.getServers()) {
            if (server != null && server.isAlive() && server.getPort() > 0 && server.getTimer().averageInMillis() > highestTickTime) {
                highestTickTime = server.getTimer().averageInMillis();
            }
        }
        ...
        for (ServerInfo server : ServerInfo.getServers()) {
            if (server != null && server.isAlive() && server.getPort() > 0) {
                int count = (int) (highestTickTime - server.getTimer().averageInMillis() + 1);
        ...
        ServerInfo bestServer = servers.get((int) (Math.random() * servers.size()));
